...
- **Changed:**
  ...
  - **`src/styles.css`:** added the full support stylesheet (modal, coffee CTA, tier ladder, BTC copy row, Summary strip) — these classes had no styles before, matched to the existing design tokens. Follow-up fix: the global `a:hover{text-decoration:underline}` was leaking onto the tier/coffee/patreon anchors on hover; pinned `text-decoration:none` on their hover states.
...
- **Verified:** `lingua.levelbrook.com` returns HTTP 200 serving bundle `index-NLGuuB_g.js`; grepped the live JS and confirmed all three support values present. Browser-tested the live modal with cached Playwright Chromium (headless, 412×915): Menu → "☕ Support" opens the modal; asserted coffee href = BMC handle, 4 tiers with Polyglot featured, Patreon button href correct, BTC row present with the right address, and (post-fix) a hovered tier card computes `text-decoration-line: none`. Screenshots captured and eyeballed.
